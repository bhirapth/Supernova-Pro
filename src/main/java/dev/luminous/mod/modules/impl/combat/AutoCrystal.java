...
public class AutoCrystal extends Module {
    ...
    private void getCrystalPos() {
        ...
        ArrayList<PlayerAndPredict> targets = getValidTargets();
        ...
        if (targets.isEmpty()) {
            ...
            findPlacePosition(targets, self);
            findBreakPosition(targets, self);
            ...
            handleAntiSurround(targets, self);
        }
    ...
    private ArrayList<PlayerAndPredict> getValidTargets() {
    ...
    private boolean isValidCrystalDamage(float damage, float selfDamage, PlayerEntity target) {
        if (selfDamage > maxSelf.getValue()) return false;
        if (noSuicide.getValue() > 0 && selfDamage > mc.player.getHealth() + mc.player.getAbsorptionAmount() - noSuicide.getValue()) return false;

        if (damage < EntityUtil.getHealth(target)) {
            double requiredDamage = getDamage(target);
            if (damage < requiredDamage) return false;
            if (smart.getValue()) {
                if (requiredDamage == forceMin.getValue()) {
                    if (damage < selfDamage - 2.5) return false;
                } else {
                    if (damage < selfDamage) return false;
                }
            }
        }
        return true;
    ...
    private void findPlacePosition(ArrayList<PlayerAndPredict> targets, PlayerAndPredict self) {
        for (BlockPos pos : BlockUtil.getSphere((float) range.getValue() + 1)) {
            if (behindWall(pos)) continue;
            if (mc.player.getEyePos().distanceTo(pos.toCenterPos().add(0, -0.5, 0)) > range.getValue()) continue;
            if (!canTouch(pos.down())) continue;
            if (!canPlaceCrystal(pos, true, false)) continue;

            for (PlayerAndPredict pap : targets) {
                ...
                if (!isValidCrystalDamage(damage, selfDamage, pap.player)) continue;
    ...
    private void findBreakPosition(ArrayList<PlayerAndPredict> targets, PlayerAndPredict self) {
        for (Entity entity : mc.world.getEntities()) {
            if (!(entity instanceof EndCrystalEntity crystal)) continue;
            if (!mc.player.canSee(crystal) && mc.player.getEyePos().distanceTo(crystal.getPos()) > wallRange.getValue()) continue;
            if (mc.player.getEyePos().distanceTo(crystal.getPos()) > range.getValue()) continue;
    ...
    private void handleAntiSurround(ArrayList<PlayerAndPredict> targets, PlayerAndPredict self) {
        if (!antiSurround.getValue()) return;
        if (PacketMine.getBreakPos() == null || PacketMine.progress < 0.9) return;
        if (BlockUtil.hasEntity(PacketMine.getBreakPos(), false)) return;
        if (tempDamage > antiSurroundMax.getValueFloat()) return;
        ...
        for (PlayerAndPredict pap : targets) {
            for (Direction dir : Direction.values()) {
                ...
                if (tryPlaceAtAntiSurround(offsetPos.offset(dir), self)) return;
                tryAdjacentPositions(offsetPos, dir, self);
            }
        }
    }

    private boolean tryPlaceAtAntiSurround(BlockPos pos, PlayerAndPredict self) {
        if (!canPlaceCrystal(pos, false, false)) return false;
        float selfDamage = calculateDamage(pos, self.player, self.predict);
        if (!isValidSelfDamage(selfDamage)) return false;
    ...
    private void tryAdjacentPositions(BlockPos offsetPos, Direction originalDir, PlayerAndPredict self) {
        for (Direction adjDir : Direction.values()) {
            if (adjDir == Direction.DOWN || adjDir == originalDir) continue;

            BlockPos adjPos = offsetPos.offset(adjDir);
            if (!canPlaceCrystal(adjPos, false, false)) continue;

            float selfDamage = calculateDamage(adjPos, self.player, self.predict);
            if (!isValidSelfDamage(selfDamage)) continue;
    ...
    private boolean isValidSelfDamage(float selfDamage) {
        return selfDamage < maxSelf.getValue() && !(noSuicide.getValue() > 0 && selfDamage > mc.player.getHealth() + mc.player.getAbsorptionAmount() - noSuicide.getValue());
    }
